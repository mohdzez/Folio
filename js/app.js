...
import { seedDemo, getTasks, getCategories, toggleDone, deleteTask, archiveTask, updateTask, exportData, importData, getPrefs, savePrefs } from './db.js';
import { renderPage, renderCategories, renderHeaderDate, updateNavCounts, renderSearch, renderAll } from './render.js';
import { openAddTask, openEditTask, handleToggle, handleDelete, handleArchive, initTaskSheet } from './tasks.js';
import { openCatModal, initCatSheet, handleDeleteCategory } from './categories.js';
import { openNotesPanel, initNotesPanel } from './notes.js';
import { openSchedulerModal, closeSchedulerModal, initScheduler, scheduleDailyDigest } from './scheduler.js';
...
const PAGE_TITLES = {
  ...
  'cat-detail': () => getCategories().find(c => c.id === catId)?.name ?? 'Category',
  archive:     'Archive',
  search:      'Search',
};
...
function navigate(newPage, opts={}) {
  ...
  document.querySelectorAll('.nav-tab[data-page]').forEach(t =>
    t.classList.toggle('active', t.dataset.page === newPage)
  );
  document.getElementById(`page-${newPage}`)?.classList.add('active');

  const t = PAGE_TITLES[newPage];
  document.getElementById('header-title').textContent = typeof t === 'function' ? t() : t;
...
initSwipe(content, {
  onDone: id => { handleToggle(id, refresh); },
  onArchive: id => { handleArchive(id, refresh); },
});
...
content.addEventListener('click', e => {
  ...
  if (bulkMode) {
    ...
    if (wrap) { toggleBulkSelect(wrap.dataset.taskId); return; }
  }

  // Toggle done
  const toggleEl = e.target.closest('[data-toggle]');
  if (toggleEl) { handleToggle(toggleEl.dataset.toggle, refresh); return; }

  // Open notes
  const notesEl = e.target.closest('[data-notes]');
  if (notesEl) { openNotesPanel(notesEl.dataset.notes, refresh); return; }

  // Category card
  const catCard = e.target.closest('.cat-card[data-cat-id]');
  if (catCard && !e.target.closest('[data-delete-cat]')) {
    navigate('cat-detail', { catId: catCard.dataset.catId }); return;
  }
...
document.getElementById('fab-btn')?.addEventListener('click', () => openAddTask());
...
document.getElementById('settings-sheet-overlay')?.addEventListener('click', e => {
  if (e.target.id === 'settings-sheet-overlay') document.getElementById('settings-sheet-overlay').classList.remove('open');
});
...
document.addEventListener('keydown', e => {
  ...
  ['task-sheet-overlay','cat-sheet-overlay','filter-sheet-overlay',
   'sched-sheet-overlay','settings-sheet-overlay'].forEach(id => {
...
function toggleBulkSelect(taskId) {
  ...
  if (bulkSel.has(taskId)) { bulkSel.delete(taskId); wrap?.classList.remove('selected'); }
  else                      { bulkSel.add(taskId);    wrap?.classList.add('selected'); }
  document.getElementById('bulk-count').textContent = `${bulkSel.size} selected`;
...
document.getElementById('bulk-delete-btn')?.addEventListener('click', () => {
  const ids = [...bulkSel];
  const deleted = ids.map(id => getTasks().find(t => t.id === id)).filter(Boolean);
  ids.forEach(id => deleteTask(id));
  showToast(`${ids.length} deleted`, () => {
    deleted.forEach(t => {
      import('./db.js').then(m => { m.addTask(t); refresh(); });
    });
  });
  exitBulkMode(); refresh();
});
...
if (digestToggle) {
  ...
  digestToggle.addEventListener('change', async e => {
    ...
    if (on) {
      ...
      } catch { digestToggle.checked = false; }
    } else {
...
// ── SW MESSAGES ───────────────────────────────
initMessageListener({
  onMarkDone: id => { handleToggle(id, refresh); },
});
...
async function boot() {
  ...
  const prefs = getPrefs();
  if (prefs.dailyDigest && Notification.permission === 'granted') scheduleDailyDigest();
