...
const REMOTE_PUSH_WORKER_URL =
  'https://folio-push-worker.munimahmad2.workers.dev';
...
function syncSW(tasks) {
  ...
  // Best-effort sync for Cloudflare cron push backend.
  // This keeps worker KV up to date so scheduled checks can send reminders.
  fetch(`${REMOTE_PUSH_WORKER_URL}/sync-tasks`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ tasks }),
  }).catch(() => {
    // Silently ignore network failures; local app behavior should continue.
  });
}
  ...
  saveTasks([
    {
      ...
      notes: 'Check Q2 objectives and budget breakdown',
      due: now,
      priority: 'high',
      cat: 'work',
      tags: ['q2', 'review'],
      recur: 'none',
      subtasks: [
        { id: 's1', title: 'Read exec summary', done: true },
        { id: 's2', title: 'Review budget', done: false },
      ],
      done: false,
      archived: false,
      created: Date.now(),
    ...
    },
    {
      ...
      recur: 'none',
      subtasks: [
        { id: 's3', title: 'Milk', done: false },
        { id: 's4', title: 'Coffee', done: false },
      ],
      done: false,
      archived: false,
      created: Date.now(),
    ...
    },
    {
      id: 'd6',
      title: 'Team standup',
      notes: 'Prepare updates for the week',
      due: add(2),
      priority: 'med',
      cat: 'work',
      tags: ['meetings'],
