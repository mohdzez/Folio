...
import { isToday, isOverdue, formatDate, renderMarkdown, escHtml, showToast } from './utils.js';
...
export function openFocusMode(taskId, { onClose, onEdit, onSched, onDone }) {
  _taskId  = taskId;
  _onClose = onClose;
  _onEdit  = onEdit;
  _onSched = onSched;
...
function renderFocus() {
  ...
  const dueLabel = task.due ? (isToday(task.due) ? 'Today' : formatDate(task.due)) : '';
  const priLabels = { high:'⬤ High priority', med:'⬤ Medium', low:'⬤ Low', none:'' };
  const priColors = { high:'var(--red)', med:'var(--gold)', low:'var(--text-muted)', none:'' };
  const recurLabel = task.recur && task.recur !== 'none' ? `↻ ${task.recur}` : '';

  document.getElementById('focus-meta-row').innerHTML = [
    dueLabel ? `<span class="focus-meta-badge task-due ${dueClass}">◷ ${dueLabel}</span>` : '',
    task.priority !== 'none' ? `<span class="focus-meta-badge" style="color:${priColors[task.priority]}">${priLabels[task.priority]}</span>` : '',
    recurLabel ? `<span class="focus-meta-badge" style="color:var(--blue)">${recurLabel}</span>` : '',
    ...(task.tags||[]).map(tag => `<span class="focus-meta-badge">#${escHtml(tag)}</span>`),
  ].filter(Boolean).join('');

  // Subtasks
  const subs = task.subtasks || [];
  document.getElementById('focus-subtasks').innerHTML = subs.map(s => `
...
export function initFocusMode(callbacks={}) {
  ...
  document.getElementById('focus-launch-btn')?.addEventListener('click', () => {
    ...
    const target = tasks.find(t => isToday(t.due)) || tasks.find(t => isOverdue(t.due)) || tasks[0];
