...
export async function updateNotifButton() {
  ...
  if (status==='granted') { label.textContent='Notifications on ✓'; btn.style.color='var(--green)'; btn.onclick=null; }
  else if (status==='denied') { label.textContent='Notifications blocked'; btn.style.opacity='.5'; btn.onclick=null; }
  else {
    label.textContent='Enable Notifications';
    btn.onclick = async () => {
      try { await requestAndSubscribe(); updateNotifButton(); }
      catch { label.textContent='Permission denied'; }
    };
  }
}
