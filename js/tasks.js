...
import { getTasks, getCategories, addTask, updateTask, deleteTask, archiveTask, toggleDone, toggleSubtask } from './db.js';
...
export function saveTask() {
  ...
  if (!title) { document.getElementById('t-title')?.focus(); return false; }

  const payload = {
    title, priority:_priority, cat:_cat, recur:_recur,
    tags:     [..._tags],
    subtasks: [..._subtasks],
  ...
  if (_editingId) {
    ...
    addTask({ id:newId('task'), ...payload, notes:'', done:false, archived:false, created:Date.now() });
...
export function initTaskSheet(onSave) {
  document.getElementById('task-save-btn')?.addEventListener('click', () => { if(saveTask()) onSave(); });
  document.getElementById('task-cancel-btn')?.addEventListener('click', closeSheet);
  ...
  tagInput?.addEventListener('keydown', e => {
    if (e.key === 'Enter' || e.key === ',') {
      ...
      if (val && !_tags.includes(val)) { _tags.push(val); renderTagPills(_tags); }
      tagInput.value = '';
    }
    if (e.key === 'Backspace' && !tagInput.value && _tags.length) {
      _tags.pop(); renderTagPills(_tags);
    }
  });
...
function renderPriSeg(val) {
  document.querySelectorAll('#t-priority-seg .seg-btn').forEach(b =>
...
function renderRecurSeg(val) {
  document.querySelectorAll('#t-recur-seg .seg-btn').forEach(b =>
    b.classList.toggle('active', b.dataset.val === val)
  );
  _recur = val;
}
...
function renderCatPills(selected) {
  ...
  document.getElementById('t-cat-pills').innerHTML = cats.map(c => `
