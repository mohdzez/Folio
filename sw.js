...
function appUrl(path = './') {
  ...
  if (path.startsWith('/')) return `${scope.origin}${basePath}${path.slice(1)}`;
  return new URL(path, scope.href).href;
}
...
self.addEventListener('notificationclick', (e) => {
  ...
  if (action === 'done' && taskId) {
    clients
      ...
      .then((ws) =>
        ws.forEach((w) => w.postMessage({ type: 'MARK_DONE', taskId })),
      );
    return;
  }
  e.waitUntil(
...
self.addEventListener('message', (e) => {
  ...
  if (type === 'SCHEDULE_NOTIF') {
    armTimer(e.data.schedule);
    persistSched(e.data.schedule);
  }
  if (type === 'CANCEL_NOTIF') {
    clearTimeout(_timers.get(e.data.schedId));
    _timers.delete(e.data.schedId);
    removeSched(e.data.schedId);
  }
});
...
const CACHE = 'folio-v4';
  ...
  const now = Date.now();
  tasks
    .filter((t) => !t.done && !t.archived && t.due)
    .forEach(async (t) => {
      ...
      if (dueMs > now && dueMs - now < 60 * 60 * 1000) {
        await self.registration.showNotification('⏰ Due soon — Folio', {
          ...
          icon: 'assets/icons/icon-192.png',
          tag: 'folio-' + t.id,
          data: { taskId: t.id, url: './?page=today' },
        });
      }
    });
}
